...
class Computers(tag: Tag) extends Table[Computer](tag, "computer") {
  ...
  //implicit val dateColumnType = MappedColumnType.base[Date, SqlDate](d => new SqlDate(d.toDate().getTime()), d => new Date(d.getTime()))
  implicit val javaUtilDateTypeMapper = MappedColumnType.base[java.util.Date, java.sql.Date](
...
object Computers {
   ...
   * @param filter
   */
  def list(page: Int = 0, pageSize: Int = 5, orderBy: Int = 1, filter: String = "%")(implicit s: Session): Page[(Computer, Option[Company])] = {
